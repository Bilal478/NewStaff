@props(['team', 'users', 'usersCount'])

<div class="w-full md:w-1/2 lg:w-1/3 xl:w-1/4">
    <article
        wire:click="teamShow({{$team->id}})"
            ...
            <div class="mb-4 flex items-center justify-end">
                @role(['owner', 'manager'])
                    <x-dropdowns.context-menu class="-mr-2">
                        <x-dropdowns.context-menu-item wire:click.stop="$emit('teamsEdit', {{$team->id}})" name="Edit" svg="svgs.edit"/>
                        <x-dropdowns.context-menu-item wire:click.stop="teamArchive({{$team->id}})" name="Delete" svg="svgs.trash"/>

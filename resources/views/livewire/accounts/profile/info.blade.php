<div x-data="{ open: false }" class="relative border-t">
    ...
    >
        <ul class="text-gray-700 text-xs">
            <li class="p-1">
                <a href="{{ route('profile.edit') }}" class="px-3 py-1 rounded-md leading-6 flex items-center hover:bg-gray-100 hover:text-blue-600">
                    <x-svgs.user class="w-4 h-4 mr-2" />
                    <span class="text-gray-700">My Profile</span>
                </a>
            </li>
            <li class="p-1">
                <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="px-3 py-1 rounded-md leading-6 flex items-center hover:bg-gray-100 hover:text-blue-600">
